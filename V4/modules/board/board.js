 ...
 * Point d'entrée de l'éditeur tactique
 */
window.ORB = window.ORB || {};

document.addEventListener('DOMContentLoaded', async () => {
    // 1. Applique les thèmes
    ...
    // 3. Lance les modules de l'éditeur
    ORB.state.init();
    ORB.renderer.init();
    ORB.interaction.init();
    ORB.ui.init();
    ORB.animation.init();

    // 4. CHARGEMENT DEPUIS LA BIBLIOTHÈQUE
    // Si on a cliqué sur "Ouvrir" dans la bibliothèque, on récupère l'ID
    const pendingLoadId = localStorage.getItem('orb_pending_load_id');
    
    if (pendingLoadId) {
        localStorage.removeItem('orb_pending_load_id'); // On nettoie pour la prochaine fois
        const playbook = await orbDB.getPlaybook(parseInt(pendingLoadId, 10));
        
        if (playbook && playbook.playbookData) {
            ORB.state.loadState(playbook.playbookData);
            document.getElementById('play-name-input').value = playbook.name || '';
            window.ORB.currentPlaybookId = playbook.id; // Stocke l'ID pour écraser lors de la sauvegarde
        }
    } else {
        window.ORB.currentPlaybookId = null; // C'est un nouveau schéma vierge
    }
    
    ORB.renderer.redrawCanvas();
});